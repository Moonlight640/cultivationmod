...
public abstract class CultivationTab {
    protected final Minecraft minecraft;
    protected final CultivationScreen screen;
    private final CultivationTabType type;
    ...
    public CultivationTab(Minecraft minecraft, CultivationScreen screen, CultivationTabType type, int index, int page, ItemStack icon, Component title, ResourceLocation background, boolean scrollable) {
    ...
    public CultivationScreen getScreen() {
        return this.screen;
    }
    ...
    public void scroll(double pDragX, double pDragY) {
        ...
        if (this.maxX - this.minX > CultivationScreen.WINDOW_INSIDE_WIDTH) {
            this.scrollX = Mth.clamp(this.scrollX + pDragX, -(this.maxX - CultivationScreen.WINDOW_INSIDE_WIDTH), 0.0D);
        }
        if (this.maxY - this.minY > CultivationScreen.WINDOW_INSIDE_HEIGHT) {
            this.scrollY = Mth.clamp(this.scrollY + pDragY, -(this.maxY - CultivationScreen.WINDOW_INSIDE_HEIGHT), 0.0D);
        }
    }
    ...
    public CultivationTabType getType() {
        return this.type;
    }
    ...
    public void drawContents(GuiGraphics pGuiGraphics, int pX, int pY) {
        ...
        pGuiGraphics.enableScissor(pX, pY, pX + CultivationScreen.WINDOW_INSIDE_WIDTH, pY + CultivationScreen.WINDOW_INSIDE_HEIGHT);
        ...
        for (int i1 = -1; i1 <= 15; ++i1) {
            for (int j1 = -1; j1 <= 8; ++j1) {
                pGuiGraphics.blit(this.background, k + CultivationScreen.BACKGROUND_TILE_WIDTH * i1, l + CultivationScreen.BACKGROUND_TILE_HEIGHT * j1, 0.0F, 0.0F,
                        CultivationScreen.BACKGROUND_TILE_WIDTH, CultivationScreen.BACKGROUND_TILE_HEIGHT, CultivationScreen.BACKGROUND_TILE_WIDTH, CultivationScreen.BACKGROUND_TILE_HEIGHT);
            }
        }
